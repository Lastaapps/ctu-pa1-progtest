...
compileOptionsDef="-std=c++11 -g -Wall -Werror -pedantic -O2 -Wno-long-long -fsanitize=address"
...
findTestData() {
	...
	count=$(ls -1 "${searchPath}"/*/CZE/$testFileFormat "${searchPath}"/*/ENG/$testFileFormat 2>/dev/null | wc -l)

	if [ $((count)) -gt 0 ]; then

		echo Found subdirectories, calling them for testing

		for subDir in ${searchPath}/*
		do
			echo Calling progtest.sh "${argsFile}" "${subDir}"
			./progtest.sh "${argsFile}" "${subDir}"
			code=$?
